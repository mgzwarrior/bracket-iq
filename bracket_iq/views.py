...
def create_bracket_form(request):
    ...
    tournaments = Tournament.objects.all().order_by('-year')
    return render(request, 'bracket_iq/create_bracket.html', {
...
def create_bracket_from_form(request):
    ...
    bracket = Bracket.objects.create(user=request.user, tournament=seedlist.tournament)
    ...
    for i in range(1, 69):
        ...
        game = Game.objects.create(
            ...
            round=round,
            bracket=bracket,
            tournament=seedlist.tournament,
            game_number=i,
            region=region
        )
        ...
        if game_id and winner_id:
            ...
            with transaction.atomic():
                ...
                if game.next_game:
                    ...
                    other_game = Game.objects.filter(
                        next_game=next_game
                    ).exclude(id=game.id).first()
...
def view_bracket(request, bracket_id):
    bracket = get_object_or_404(Bracket, id=bracket_id)
    predictions = Prediction.objects.filter(bracket=bracket).select_related('game', 'predicted_winner')
...
def seed_tournament(request):
    ...
    years = range(current_year, current_year + 5)  # Allow seeding up to 5 years in advance

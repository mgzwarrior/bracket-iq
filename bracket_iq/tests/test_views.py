...
from ..models import Team, Bracket, Game, Prediction, SeedList, Seed, Round, Tournament
...
class ViewTests(TestCase):
    ...
    def setUp(self):
        ...
        self.seed_list = SeedList.objects.create(tournament=self.tournament)
        self.seed1 = Seed.objects.create(team=self.team1, seed=1, true_seed=1, seed_list=self.seed_list)  # A top seed!
        self.seed2 = Seed.objects.create(team=self.team2, seed=16, true_seed=2, seed_list=self.seed_list)  # The underdog
    ...
    def test_create_bracket_view(self):
        ...
        self.client.login(username='testuser', password='12345')
        response = self.client.post(reverse('create_bracket', args=[self.seed_list.id]))
    ...
    def test_create_live_bracket_view(self):
        ...
        self.assertEqual(response.status_code, 302)  # Redirect to next game or display_bracket 